...
@php
    ...
    $ic = 'w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 placeholder-gray-400 dark:placeholder-gray-500';
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div class="p-6 space-y-4">
            ...
            <div><label class="{{ $lc }}">Tipo de Rede</label><select wire:model.live="form_network_type_id" class="{{ $sc }}"><option value="">Selecione...</option>@foreach($selects['network_types'] as $id=>$name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div>
            @if($this->showBandwidth())<div><label class="{{ $lc }}">Largura de Banda</label><select wire:model.live="form_bandwidth_option_id" class="{{ $sc }}"><option value="">Selecione...</option>@foreach($selects['bandwidth_options'] as $id=>$name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div>@endif
            <div><label class="{{ $lc }}">IPs Públicos Adicionais</label><input type="number" wire:model="form_extra_public_ips" min="0" max="20" class="{{ $ic }}" placeholder="0"/><p class="text-[10px] text-gray-500 mt-1">Além do IP padrão incluído gratuitamente.</p></div>
        </div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div class="p-6 space-y-4"><div><label class="{{ $lc }}">Armazenamento desejado (GB) *</label><input type="number" wire:model="form_s3_storage_gb" min="1" class="{{ $ic }}" placeholder="100"/><p class="text-[10px] text-gray-500 mt-1">Você pode aumentar a qualquer momento.</p></div><div><label class="{{ $lc }}">Observações</label><textarea wire:model="form_s3_notes" rows="3" class="{{ $ic }}" placeholder="Uso pretendido, projeto relacionado..."></textarea></div></div>
    ...
    <div class="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <div><label class="{{ $lc }}">Tipo de Rede *</label><select wire:model="form_bkp_network" class="{{ $sc }}"><option value="">Selecione...</option>@foreach($selects['network_types'] as $id=>$name)<option value="{{ $id }}">{{ $name }}</option>@endforeach<option value="vpn_client">VPN Client</option></select></div>
                <div><label class="{{ $lc }}">Banda</label><select wire:model="form_bkp_bandwidth_id" class="{{ $sc }}"><option value="">Selecione...</option>@foreach($selects['bandwidth_options'] as $id=>$name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div>
            </div>
            <div>
                ...
                <div class="flex gap-2 items-center mb-2">
                    <input type="text" wire:model.lazy="form_bkp_machines_detail.{{ $idx }}.descricao" placeholder="Nome da máquina" class="{{ $ic }}"/>
                    <input type="number" wire:model.lazy="form_bkp_machines_detail.{{ $idx }}.disk_gb" min="10" placeholder="GB" class="w-24 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary-500 shrink-0"/>
            ...
            <div class="grid grid-cols-2 gap-4"><div><label class="{{ $lc }}">Retenção</label><select wire:model="form_bkp_retention_id" class="{{ $sc }}"><option value="">Selecione...</option>@foreach($selects['retention_options'] as $id=>$name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div><div><label class="{{ $lc }}">Software de Backup</label><select wire:model="form_bkp_software_id" class="{{ $sc }}"><option value="">Selecione...</option>@foreach($selects['backup_sw_options'] as $id=>$name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div></div>
        </div>
        ...
        <div class="px-6 py-4 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <select wire:model.live="proposta_currency" class="w-full rounded-lg bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm px-3 py-2 focus:ring-1 focus:ring-emerald-500 outline-none">
